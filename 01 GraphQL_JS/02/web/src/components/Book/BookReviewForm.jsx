...
const BookReviewForm = (props) => {
  const { book, reviewInput = {}, handleChange, handleSubmit, inputValid } = props;
  const { count, hoverCount, name, email, title: commentTitle, comment } = reviewInput;
  if (!book) return null;
  return (
    <form onSubmit={handleSubmit} className="mv3 f4 black-80">
      ...
      />
      <InputSet label="Display Name" type="text" value={name} onChange={(e) => handleChange("name", tv(e))} />
      <InputSet label="Email" type="text" value={email} onChange={(e) => handleChange("email", tv(e))} />
      <InputSet label="Title" type="text" value={commentTitle} onChange={(e) => handleChange("title", tv(e))} />
      <TextAreaSet label="Comments" value={comment} onChange={(e) => handleChange("comment", tv(e))} />
      <div className="measure-narrow gray">
        <input
          disabled={!inputValid}
          className={classNames("b ph3 pv2 input-reset ba bg-transparent f6 dib", {
