...
create function hb.create_book(
  ...
  authors text [],
  page_count integer
) returns hb.book as $ $ declare book hb.book;
...
from
  hb.book
where
  hb.book.google_id = google_book_id into book;

if book.id > 0 then return book;

else tokens := to_tsvector(
...
insert into
  hb.book_author(book_id, author_id)
select
  book.id,
  id
from
...
create function hb.create_review(
  ...
  title text,
  comment text
) returns hb.review as $ $ declare user_id integer;
...
begin
insert into
  hb.user(email, name, tokens)
values
  (reviewer_email, name, to_tsvector(name)) on conflict (email) do nothing;
...
insert into
  hb.review(user_id, book_id, rating, title, comment, tokens)
values
  (
    user_id,
    book_id,
